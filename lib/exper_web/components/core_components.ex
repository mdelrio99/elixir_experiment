defmodule ExperWeb.CoreComponents do
  @moduledoc """
  ...
  def table(assigns) do
    assigns =
      ...
      #mt-11 removed from table class
      #thead is now hidden
      #tbody border-t => tr border-b
      #bg-zinc-50 => bg-slate-500
      #text-zinc-900 => text-white-900
      #removed from span group-hover: sm:rounded-l-xl
    ~H"""
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0" style="padding-right:5px">
      <table class="w-[40rem] sm:w-full" >
        <thead class="text-left text-[0.8125rem] leading-6 text-zinc-200" style="display:">
          <tr>
  ...
  def table_dark(assigns) do
    assigns =
      ...
      #mt-11 removed from table class
      #thead is now hidden
      #tbody border-t => border-b
    ~H"""
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="w-[40rem] sm:w-full" >
        <thead class="text-left text-[0.8125rem] leading-6 text-zinc-500" style="display:none">
          <tr>
            <th :for={col <- @col} class="p-0 pb-4 pr-6 font-normal"><%= col[:label] %></th>
            <th class="relative p-0 pb-4"><span class="sr-only"><%= gettext("Actions") %></span></th>
          </tr>
        ...
        <tbody
          id={@id}
          phx-update={match?(%Phoenix.LiveView.LiveStream{}, @rows) && "stream"}
          class="relative divide-y divide-zinc-100 border-b border-zinc-200 text-sm leading-6 text-zinc-700"
          >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ExperWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ExperWeb.Gettext, "errors", msg, opts)
    end
  end
