defmodule ExperWeb.TodoLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      />
      <.input field={@form[:datecompleted]} type="date" label="Date Completed" />
      <:actions>
  ...
  def update(%{todo: todo} = assigns, socket) do
    changeset = Library.change_todo(todo)
  ...
  def handle_event("validate", %{"todo" => todo_params}, socket) do
    changeset =
      socket.assigns.todo
      |> Library.change_todo(todo_params)
  ...
  defp save_todo(socket, :edit, todo_params) do
    case Library.update_todo(socket.assigns.todo, todo_params) do
      {:ok, todo} ->
  ...
  defp save_todo(socket, :new, todo_params) do
    case Library.create_todo(todo_params) do
      {:ok, todo} ->
        notify_parent({:saved, todo})
